...
import (
	...
	"os/user"
	"path"
	"prismx_cli/utils/go-ora/advanced_nego"
	"prismx_cli/utils/go-ora/network"
	"prismx_cli/utils/go-ora/trace"
	"runtime"
	"strconv"
	"strings"
	"time"
...
func newConnectionStringFromUrl(databaseUrl string) (*ConnectionString, error) {
	...
	for key, val := range q {
		...
		case "FAILOVER":
			return nil, errors.New("starting from go-ora.7.0 this feature (FAILOVER) is not supported and the driver use database/sql package fail over")
		...
		case "RE-TRY TIME":
			fallthrough
		case "RETRY TIME":
			return nil, errors.New("starting from go-ora.7.0 this feature (RETRY TIME) is not supported and the driver use database/sql package fail over")
