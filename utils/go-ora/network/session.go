...
import (
	...
	"prismx_cli/utils/go-ora/converters"
)
...
func (session *Session) Connect(ctx context.Context) error {
	...
	for loop {
		...
		if len(session.Context.ConnOption.UnixAddress) > 0 {
			session.conn, err = netUtils.SendDialTimeout("unix", session.Context.ConnOption.UnixAddress, session.Context.ConnOption.Timeout)
		} else {
			session.conn, err = netUtils.SendDialTimeout("tcp", addr, session.Context.ConnOption.Timeout)
		}
